...
export default async function ClaimsPage() {
  ...
  return (
    <div className="space-y-6 animate-fade-up">
      <PageHeader
        title="Klaim"
        description="Klaim kepemilikan yang Anda ajukan dan klaim masuk atas barang temuan Anda."
      />
      ...
      {items.length === 0 ? (
        <EmptyState
          ...
          description="Klaim muncul di sini saat Anda mengajukan kepemilikan atas potential match, atau saat seseorang mengklaim barang yang Anda temukan."
        />
      ) : (
            ...
            >
              <Card className="h-full p-5 transition-all duration-200 group-hover:shadow-lift group-hover:ring-brand-200">
                <div className="flex items-start justify-between gap-2">
                  ...
                  >
                    {role === "claimant" ? "Klaim Saya" : "Klaim Masuk"}
                  </Badge>
                ...
                <div className="mt-4 flex items-center justify-between border-t border-slate-100 pt-3">
                  <span className="text-xs font-semibold text-slate-500">
                    {claim.verification_score !== null
                      ? `Verification score: ${Math.round(Number(claim.verification_score))}%`
